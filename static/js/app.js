...
function bindUI() {
    ...
    Object.keys(taxonomyUI).forEach(kind => {
        ...
        input.addEventListener("change", () => addTagFromInput(kind));
    });
...
async function loadTaxonomy() {
    await Promise.all(Object.keys(taxonomyUI).map(async kind => {
        let snap = await getDocs(collection(db, kind));
        if (snap.empty) {
            await Promise.all(defaultTaxonomy[kind].map(name => setDoc(doc(db, kind, taxonomyDocId(name)), {
                name, normalized: normalizeTaxonomyValue(name), active: true, createdBy: currentUser.uid, createdAt: serverTimestamp()
            }).catch(() => null)));
            snap = await getDocs(collection(db, kind));
        }
        taxonomy[kind] = snap.docs.map(d => d.data()).filter(item => item.active !== false && item.name).map(item => item.name).sort((a,b) => a.localeCompare(b));
        renderTaxonomyOptions(kind);
    }));
}

function renderTaxonomyOptions(kind) {
    const datalist = $(taxonomyUI[kind].options);
    datalist.replaceChildren(...taxonomy[kind].filter(name => !selectedTags[kind].some(v => normalizeTaxonomyValue(v) === normalizeTaxonomyValue(name))).map(name => {
...
async function addTagFromInput(kind) {
    ...
    if (!existing) {
        ...
        if (!snap.exists()) await setDoc(ref, { name, normalized, active: true, createdBy: currentUser.uid, createdAt: serverTimestamp() });
